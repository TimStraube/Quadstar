...
\begin{tikzpicture}[node distance=2cm, auto, >=latex', thick]
  ...
  \node[font=\Huge\bfseries] at (0,12) {Quadstar System Engineering Architecture};
  
  % Hauptdiagramm mit Offset
  \begin{scope}[xshift=2cm, yshift=9cm]
  % Vertikale Anordnung: Joystick, QuadUI, Communication Units, MCU
  \node[draw, rounded rectangle, fill=gray!20] (joystick) {\#44 Joystick};
  \node[draw, rounded rectangle, right=2.5cm of joystick, fill=green!10] (quadui) {\#16 User Interface};
  \node[draw, rounded rectangle, below=2cm of joystick, fill=blue!10] (gcu) {\#43 Ground Communication Unit (GCU)};
  \node[draw, rounded rectangle, below=2cm of gcu, fill=blue!10] (rx) {\#30 Quad Communication Unit (QCU)};
  \node[draw, rounded rectangle, below=2cm of rx, fill=blue!10, minimum width=3cm] (mcu) {\#40 Main Compute Unit (MCU)};
  ...
  \node[draw, rounded rectangle, fill=blue!10, left=2.5cm of pwh, minimum width=2.5cm] (akku) {\#21 Akku};
  \node[draw, rounded rectangle, fill=gray!20, left=4.5cm of akku, minimum width=2.5cm] (ladestation) {\#45 Loading Station};
  \node[draw, rounded rectangle, fill=orange!20, below=2cm of ladestation, minimum width=2.5cm] (testbench) {\#46 Testbench};
  \draw[dashed, thick] (ladestation) -- node[above, font=\small]{Loading} (akku);
  \draw[->, thick] (akku) -- (pwh);

  % ESC and Motor (single representation for 4 units)
  \node[draw, rounded rectangle, below=1.5cm of mcu, fill=blue!10] (esc) {\#38 Electronic Speed Controller (ESC)};
  \node[draw, rounded rectangle, below=1.2cm of esc, fill=gray!20] (motor) {\#41 BLDC-Motor};
  ...
  \node[draw, rounded rectangle, right=3cm of esc, fill=gray!20] (step) {\#42 Step Motor};
  \node[right=0.2cm of step, font=\small\bfseries] {$\times 2$};
  \draw[->] (mcu) -| node[near start, above, font=\small]{s\_c} (step);
  
  % Wings
  \node[draw, rounded rectangle, below=1.2cm of step, fill=gray!20] (wings) {\#47 Wings};
  ...
  \node[draw, dashed, rounded rectangle, fit=(mcu) (esc) (motor) (rx) (akku) (step) (wings) (pwh), inner sep=1cm, label=below:{\textbf{Quadcopter}}] (quadbox) {};
