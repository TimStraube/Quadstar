...
const Simulation: React.FC = () => {
  ...
  const setupScene = () => {
    ...
    const pickable = [cube1, cube2, cube3];
    ...
    const onPointerDown = (ev: PointerEvent) => {
      try {
        ...
        console.debug('intersects count', intersects.length);
        let picked = null as any;
        for (let it of intersects) {
          let obj: any = it.object;
          // walk up parents to find pickable flag
          while (obj) {
            if (obj.userData && obj.userData.pickable) { picked = obj; break; }
            obj = obj.parent;
          }
          if (picked) break;
        }
        if (picked) {
          console.debug('picked object', picked.name || picked.type, picked);
          try { setOpenPanel('params'); } catch (e) { console.warn(e); }
        }
      } catch (e) { console.error('onPointerDown error', e); }
    };

    // register on both canvas and window as a fallback (some UI may capture events)
    try { renderer.domElement.addEventListener('pointerdown', onPointerDown); } catch(e) { console.warn('failed to add pointer listener to canvas', e); }
    try { window.addEventListener('pointerdown', onPointerDown); } catch(e) { console.warn('failed to add pointer listener to window', e); }
    ...
    function connectServer() {
      ...
      .then(data => {
        ...
        try {
          ...
          let ts: number;
          if (typeof json_data.t === 'number' && json_data.t > 1e9) ts = Number(json_data.t);
          else ts = Date.now() / 1000;
          let powerW: number | null = null;
          // 1) If backend provides voltage/current, compute electrical power
          const voltageKeys = ['voltage', 'bat_voltage', 'battery_voltage'];
          const currentKeys = ['current', 'bat_current', 'battery_current', 'current_mA', 'bat_current_mA'];
          let voltage: number | undefined;
          for (const k of voltageKeys) { if (json_data[k] !== undefined) { voltage = Number(json_data[k]); break; } }
          let current: number | undefined;
          for (const k of currentKeys) { if (json_data[k] !== undefined) { current = Number(json_data[k]); break; } }
          if (voltage !== undefined && current !== undefined && !Number.isNaN(voltage) && !Number.isNaN(current)) {
            // if current looks like mA (large number), convert to A
            if (Math.abs(current) > 500) current = current / 1000.0;
            powerW = voltage * current;
          }
          // 2) If no electrical data, try motor speeds (many backends expose `wMotor` or similar)
          if (powerW === null) {
            const motorKeys = ['wMotor', 'wmotor', 'motor_speeds', 'motor_rpms', 'motorSpeeds', 'w_motors'];
            let motors: any = null;
            for (const k of motorKeys) { if (json_data[k] !== undefined) { motors = json_data[k]; break; } }
            if (motors && Array.isArray(motors) && motors.length >= 1) {
              // determine units: assume rad/s by default; if values are large (~1000..20000) they might be RPM
              const sample0 = Number(motors[0]);
              if (!Number.isNaN(sample0)) {
                let motorsRadS = motors.map((v: any) => Number(v));
                // heuristics: if values are > 2000 assume RPM -> convert to rad/s
                if (motorsRadS.some((v: number) => Math.abs(v) > 2000)) {
                  motorsRadS = motorsRadS.map((rpm: number) => rpm * 2 * Math.PI / 60.0);
                }
                // use torque coefficient kTo if provided by backend, else fallback to default from model
                const kTo = (json_data.kTo !== undefined) ? Number(json_data.kTo) : 1.632e-7;
                // P_i = torque * omega = kTo * w^2 * w = kTo * w^3
                let totalP = 0;
                for (const w of motorsRadS) {
                  if (!Number.isNaN(w)) totalP += kTo * w * w * w;
                }
                // don't accept negative nonsense
                if (Number.isFinite(totalP) && totalP > 0) powerW = totalP;
              }
            }
          }
          ...
          if (powerW !== null && Number.isFinite(powerW)) {
            ...
            const objs = simulationObjects.current || {};
            if (!objs._lastEnergyPush || (nowMs - objs._lastEnergyPush) > 180) {
              try { console.debug('telemetry energy sample', { ts, powerW, voltage, current, motors: Array.isArray(motors) ? motors.slice(0,4) : motors }); } catch(e){}
              setEnergySamples(s => {
                const next = s.concat([{ t: ts, p: Math.max(0, powerW as number) }]);
                const cutoff = ts - 60;
                return next.filter(x => x.t >= cutoff);
              });
              objs._lastEnergyPush = nowMs;
              simulationObjects.current = objs;
            }
          }
        } catch (e) { console.debug('energy telemetry parse error', e); }
      })
      .catch((err) => {
  ...
  useEffect(() => {
    if (resetFlag) {
      ...
      try {
        const objs = simulationObjects.current;
        if (objs && objs.renderer && objs._onPointerDown) {
          try { objs.renderer.domElement.removeEventListener('pointerdown', objs._onPointerDown); } catch(e){}
